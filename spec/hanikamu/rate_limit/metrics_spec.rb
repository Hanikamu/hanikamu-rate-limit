...
RSpec.describe Hanikamu::RateLimit::Metrics do
  ...
  describe ".enabled?" do
    it "returns the metrics_enabled config value" do
  ...
  describe ".record_limit" do
    ...
    it "returns nil when metrics are disabled" do
      Hanikamu::RateLimit.configure { |c| c.metrics_enabled = false }
      result = described_class.record_limit(limit_key, { "rate" => 5, "key_prefix" => "test" })
  ...
  describe ".record_allowed" do
    ...
    it "returns nil when metrics are disabled" do
      Hanikamu::RateLimit.configure { |c| c.metrics_enabled = false }
      expect(described_class.record_allowed(limit_key)).to be_nil
    end
  end
  ...
  describe ".record_blocked" do
    ...
    it "returns nil when metrics are disabled" do
      Hanikamu::RateLimit.configure { |c| c.metrics_enabled = false }
      expect(described_class.record_blocked(limit_key)).to be_nil
    end
  end
  ...
  describe ".record_override" do
    ...
    it "returns nil when metrics are disabled" do
      Hanikamu::RateLimit.configure { |c| c.metrics_enabled = false }
      expect(described_class.record_override("test_api", 100, 3600)).to be_nil
    end
  ...
  describe ".record_registry_meta" do
    ...
    it "returns nil when metrics are disabled" do
      Hanikamu::RateLimit.configure { |c| c.metrics_enabled = false }
      expect(described_class.record_registry_meta(
               registry_limit_key, key_prefix, 10, 30, "ApiClient", "fetch"
             )).to be_nil
    end
  end
