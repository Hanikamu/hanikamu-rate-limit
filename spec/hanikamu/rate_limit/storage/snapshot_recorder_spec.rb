...
RSpec.describe Hanikamu::RateLimit::Storage::SnapshotRecorder do
  ...
  describe ".tick!" do
    let(:state_double) do
      instance_double(
        ...
        state: { current_rate: 50, consecutive_successes: 12,
                 error_ceiling: 55, ceiling_hits: 3,
                 ceiling_confidence: 2, effective_cooldown: 15.0 },
    ...
    it "records snapshots for adaptive limits when interval has elapsed" do
      ...
      expect(Hanikamu::RateLimit::Storage::RateSnapshot).to have_received(:create!).with(
        hash_including(
          ...
          error_ceiling: 55,
          ceiling_hits: 3,
          ceiling_confidence: 2,
          effective_cooldown: 15.0
        )
      )
    end
  ...
  describe ".record!" do
    let(:state_double) do
      instance_double(
        Hanikamu::RateLimit::AdaptiveState,
        state: { current_rate: 42, consecutive_successes: 7,
                 error_ceiling: nil, ceiling_hits: 0,
                 ceiling_confidence: 0, effective_cooldown: nil },
        config: { min_rate: 5, max_rate: 80 }
      )
    end

    it "creates a snapshot record with ceiling fields" do
      allow(Hanikamu::RateLimit::Storage::RateSnapshot).to receive(:create!)
      ...
      expect(Hanikamu::RateLimit::Storage::RateSnapshot).to have_received(:create!).with(
        ...
        error_ceiling: nil,
        ceiling_hits: 0,
        ceiling_confidence: 0,
        effective_cooldown: nil
      )
    end
