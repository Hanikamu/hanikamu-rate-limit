...
# Custom error class for testing adaptive error capture.
class TestApiError < StandardError; end

RSpec.describe "Adaptive rate limiting (ceiling-based integration)" do # rubocop:disable RSpec/DescribeClass
  ...
  describe "limit_method with adaptive registry" do
    ...
    it "captures error_classes exceptions via EventCapture" do
      expect(Hanikamu::RateLimit::Storage::EventCapture).to receive(:capture_exception)
        .with(:adaptive_api, an_instance_of(TestApiError), adaptive_rate: 3)
    ...
    it "re-raises the original error after capturing" do
      instance = klass.new
      instance.fail_next!
    ...
    it "passes non-error_classes exceptions through without capturing" do
      expect(Hanikamu::RateLimit::Storage::EventCapture).not_to receive(:capture_exception)
  ...
  describe "limit_method with adaptive registry and empty error_classes" do
    ...
    it "works as a rate limiter without error capture" do
      klass = Class.new do
        extend Hanikamu::RateLimit::Mixin
