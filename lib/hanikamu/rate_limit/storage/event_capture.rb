...
module Hanikamu
  module RateLimit
    module Storage
      ...
      # learning UI.  All events start as "unclassified" unless they
      # match an already-classified group (via inherit_classification).
      # Users classify events in the Learning UI, and future events with
      # the same group key auto-inherit that classification.
      #
      # When an event is classified as "rate_limit", the Learning UI
      # triggers a rate drop via AdaptiveState#drop_to_rate!.
      #
      ...
      class EventCapture
        class << self
          # Capture an exception caught by the error-capture mixin.
          #
          ...
          def capture_response(registry_name, response, parsed, adaptive_rate: nil)
            ...
            with_connection do
              attrs[:classification] = resolve_classification(attrs, parsed)
              CapturedEvent.create!(attrs)
            end
          rescue StandardError => e
            log_capture_error(e)
          ...
          def with_connection(&)
            if defined?(ActiveRecord::Base.logger) && ActiveRecord::Base.logger.respond_to?(:silence)
          ...
          # Resolves the classification for a new event by inheriting
          # from the latest classified sibling with the same group key.
          def resolve_classification(attrs, _parsed)
            inherit_classification(attrs)
          end
